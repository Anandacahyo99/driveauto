...
export default function PrivateDrivePage() {
  // --- 1. STATE MANAGEMENT (Hooks wajib di urutan teratas) ---
  ...
  const [newName, setNewName] = useState(""); // Hanya menyimpan nama depan
  const [currentExt, setCurrentExt] = useState(""); // Menyimpan ekstensi (.pdf, dll)
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setStatus({ type: "success", msg: `Tersimpan di folder: ${userName}` });
        setFiles([]);
        fetchFiles();
      } else {
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-black text-slate-900 text-xl tracking-tighter uppercase italic">
              DriveBridge
            </span>
          </div>
        ...
        <AnimatePresence mode="wait">
          {!isLoggedIn ? (
            ...
            >
              ...
              <h1 className="text-2xl font-black text-slate-900 mb-2 italic">Private Drive</h1>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <input
                  ...
                  className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-blue-500 font-bold text-slate-800 text-center outline-none transition-all"
                />
                <button
                  type="submit"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              >
                <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
                        ...
                        {renamingId === "CURRENT_USER" ? (
                          <div className="flex items-center gap-2 mt-1">
                            <input
                              ...
                              onChange={(e) => setUserName(e.target.value)}
                              onBlur={() => {
                                localStorage.setItem("drive_user", userName);
                                setRenamingId(null);
                              }}
                              className="text-sm font-bold text-slate-800 border-b border-blue-500 outline-none w-24"
                            />
                            <button onClick={() => setRenamingId(null)} className="text-emerald-500">
                              <Check size={14} />
                            </button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => setRenamingId("CURRENT_USER")} className="text-slate-300 hover:text-blue-500">
                              <Edit2 size={12} />
                            </button>
                          </div>
                        )}
                  ...
                  <form onSubmit={handleUpload} className="space-y-6">
                    <label className="group flex flex-col items-center justify-center w-full min-h-[160px] border-2 border-dashed border-slate-100 rounded-[1.5rem] cursor-pointer hover:bg-blue-50/50 hover:border-blue-300 transition-all text-center">
                      <Upload className="text-slate-300 group-hover:text-blue-600 mb-2 transition-colors mx-auto" />
                    ...
                    <button
                      ...
                      className="w-full py-4 rounded-xl font-black text-white bg-blue-600 shadow-lg shadow-blue-100 hover:bg-blue-700 disabled:bg-slate-50 disabled:text-slate-300 transition-all uppercase tracking-widest text-[10px]"
                    >
              ...
              <div className="lg:col-span-8">
                <div className="flex items-center justify-between mb-8 px-2">
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3 text-left">
                      ...
                      {driveFiles.map((file) => (
                        ...
                        >
                          <div className="flex items-center gap-4 overflow-hidden text-left w-full">
                            <div className="bg-slate-50 p-3 rounded-2xl transition-all flex-shrink-0">
                            ...
                            <div className="overflow-hidden w-full pr-2">
                              {renamingId === file.id ? (
                                <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-blue-200">
                                  <input
                                    ...
                                    className="bg-transparent outline-none font-bold text-sm text-slate-800 w-full"
                                  />
                                  <span className="text-[10px] font-black text-slate-400 bg-slate-200 px-1 rounded uppercase">.{currentExt}</span>
                                  <button onClick={() => handleRenameFile(file.id)} className="text-emerald-500 p-1">
                                    <Check size={16} />
                                  </button>
                                  <button onClick={() => setRenamingId(null)} className="text-red-500 p-1">
                                    <X size={16} />
                                  </button>
                                </div>
                              ) : (
                              ...
                              <div className="flex items-center gap-2 mt-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            <a href={file.webViewLink} target="_blank" className="p-2 text-slate-300 hover:text-blue-600 transition-colors">
                              <Eye size={18} />
                            </a>
                            <button
                            ...
                            <button onClick={() => confirmDelete(file.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors">
                              <Trash2 size={18} />
                            </button>
                          </div>
                        </motion.div>
                ...
                {driveFiles.length === 0 && !loadingFiles && (
                  <div className="text-center py-24 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 text-slate-400 font-medium italic">
      ...
      <AnimatePresence>
        {deleteId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-sm:w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 text-center">
              ...
              <p className="text-slate-500 text-sm mb-8 font-medium">Tindakan ini permanen di Google Drive.</p>
              <div className="flex gap-3">
                <button disabled={isDeleting} onClick={() => setDeleteId(null)} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold text-xs">Batal</button>
      ...
      <footer className="bg-white border-t border-slate-200 py-10 mt-auto">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between text-slate-400">
          <p className="text-[10px] font-bold uppercase tracking-widest italic">
            Bridge.Cloud Secure Transfer © 2026
          </p>
